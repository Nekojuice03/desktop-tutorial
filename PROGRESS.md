...
- **歷史局部 critic 消融**：舊文件稱為 IPPO，實作其實是共享 actor、
  ...
  Amato, arXiv:2409.03052, 2024(CTDE 專論)；arXiv:2505.03558, IEEE VNC
  2025(MEC 卸載場景 IPPO vs MAPPO——耦合強時 MAPPO 佔優 → 場景相依)。
  論文以「有理論支撐的負結果」呈現，主賣點放移動性閉環與 DT。
...
1. **擴充和平東路 replay 資料集**：
   `python collect_vd.py --scenario heping --count 0 --interval 300`，至少涵蓋平峰、
